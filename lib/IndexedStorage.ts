...
export default class IndexedStorage implements IStorageAsync {
  ...
  setAsync(key: string, value: string): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!key)
        return reject(
          "web-client-store:Indexed:setAsync::key cannot be empty/undefined"
        );

      return localforage.setItem(key, value, (err, value) => {
        if (err)
          return reject(
            `An error occured while storing the key ${key} to IndexDb (localforage). Error: ${err.message}`
          );
        resolve(value);
      });
    });
  ...
  getAsync(key: string): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!key)
        return reject(
          "web-client-store:Indexed:getAsync::key cannot be empty/undefined"
        );
      return localforage.getItem(key, (err, value) => {
        if (err)
          return reject(
            `An error occured while retrieving the key ${key} from IndexDb (localforage). Error: ${err.message}`
          );
        resolve(value);
      });
    });
  ...
  unsetAsync(key: string): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!key)
        return reject(
          "web-client-store:Indexed:unsetAsync::key cannot be empty/undefined"
        );
      return localforage.removeItem(key, (err) => {
        if (err)
          return reject(
            `An error occured while deleting the key ${key} from IndexDb (localforage). Error: ${err.message}`
          );
        resolve();
      });
    });
  ...
  clearAsync(): Promise<void> {
    return new Promise((resolve, reject) => {
      return localforage.clear((err) => {
        if (err)
          return reject(
            `An error occured while clearing all the keys from IndexDb (localforage). Error: ${err.message}`
          );
        resolve();
      });
    });
  ...
  lengthAsync(): Promise<number> {
    return new Promise((resolve, reject) => {
      return localforage.length((err, numberOfKeys) => {
        if (err)
          return reject(
            `An error occured while measuring the length of all the keys in IndexDb (localforage). Error: ${err.message}`
          );
  ...
  getKeyAtAsync(keyIndex: number): Promise<string> {
    return new Promise((resolve, reject) => {
      return localforage.key(keyIndex, (err, key) => {
        if (err)
          return reject(
            `An error occured while retrieving the key name at index ${keyIndex} from IndexDb (localforage). Error: ${err.message}`
          );
        resolve(key);
      });
    });
  ...
  getAllKeysAsync(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      return localforage.keys((err, keys) => {
        if (err)
          return reject(
            `An error occured while retrieving the all the key names from IndexDb (localforage). Error: ${err.message}`
          );
        resolve(keys);
      });
    });
  ...
  forEachAsync<T, U>(
    ...
    return new Promise((resolve, reject) => {
      return localforage.iterate(iteratee, (err, result) => {
        if (err)
          return reject(
            `An error occured while iterating over all the keys from IndexDb (localforage). Error: ${err.message}`
          );
        resolve(result);
      });
    });
